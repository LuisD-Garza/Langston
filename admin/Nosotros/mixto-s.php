...
$verificarMixto = $mysqli->query("SELECT * FROM `nosotros` WHERE nosotros.categoria = '$id'");
...
if(isset($_POST['titulo']) && isset($_POST['descripcion']))
{
	...
	if(empty($imagen))
	{
		if($cantidadMixto > 0)
		{
			...
			header("Location: " . $path . "Nosotros/textos.php?accion=e");
		} else {
...
<?php
$obtenerCategorias = $mysqli->query("SELECT * FROM `nosotros` WHERE nosotros.categoria = '$id'");
...
<div class="contenido c_<?php echo $textos['id']; ?>">
	...
	<div class="restante r_<?php echo $textos['id']; ?>" style="display: none;">
		<div class="objetos">
			...
			$obtenerObjetos = $mysqli->query("SELECT * FROM `nosotros` WHERE nosotros.id = '$idCategoria'");
			...
			<div class="objeto P">
				...
				<div class="acciones">
					<?php if ($objetos['id'] == 9 ) { ?>
					<div onClick="window.location.href='list.php?id=<?php echo $objetos['id']; ?>'" class="boton hvr-sweep-to-right" style="background: #f3c426;">Editar</div>
